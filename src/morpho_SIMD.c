...
vuint8** bords_SIMD(uint8** im, int nrl, int nrh, int ncl, int nch){
    ...
    im_mat = vui8matrix(nrl-bord, hauteur+bord, ncl-bord, largeur+bord);

    //Matrice avec l'image
    vuint8** im_vect = (vuint8**) im;
    //Premiere ligne et derniere ligne
    for(int i = ncl-b; i <= nbVuint8+b; i++){
        //TODO: vec_store ?
        vec_store(&im_mat[nrl-b][k], init_vuint8(0));
        vec_store(&im_mat[hauteur+b][k], init_vuint8(0));
    ...
    for(int j = nrl; j <= hauteur; j++){
        ...
        //Bord droit
        im_mat[j][largeur+b] = init_vuint8(0);
...
vuint8** ouverture_SIMD(uint8** im, int nrl, int nrh, int ncl, int nch){
